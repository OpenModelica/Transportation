...
model ElectricalVehiclePool
  ...
  SystemDynamics.Sources.Source source annotation(
    Placement(visible = true, transformation(origin = {-89, 54}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  SystemDynamics.Sources.Sink sink annotation(
    Placement(visible = true, transformation(origin = {91, 51}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealOutput InGrowthRate annotation(
    Placement(visible = true, transformation(origin = {77, -33}, extent = {{10, -10}, {-10, 10}}, rotation = 0), iconTransformation(origin = {-94, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealOutput InDepletionRate annotation(
    Placement(visible = true, transformation(origin = {79, -57}, extent = {{10, -10}, {-10, 10}}, rotation = 0), iconTransformation(origin = {-94, -11}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealOutput FromFossilVehicles annotation(
    Placement(visible = true, transformation(origin = {-83, -68}, extent = {{10, 10}, {-10, -10}}, rotation = -90), iconTransformation(origin = {-66, -39}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
equation
  InGrowthRate = prod_2.u1;
  InDepletionRate = prod_21.u1;
  connect(source.MassInPort1, GenerationRate.y) annotation(
    Line(points = {{-78, 54}, {-78, 16}, {-64, 16}}, color = {0, 0, 127}));
  connect(DepletionRate.y1, sink.MassInPort1) annotation(
    Line(points = {{66.25, 16.5}, {77.75, 16.5}, {77.75, 51}, {80, 51}}, color = {95, 0, 191}));
  ...
  connect(FromFossilVehicles, multiSum.u[2]) annotation(
    Line(points = {{-83, -68}, {-83, -34}, {-57, -34}}, color = {0, 0, 127}));
  annotation(
    Icon(coordinateSystem(grid = {1, 1}), graphics = {Text(origin = {3.5, -68}, extent = {{-95.5, 29}, {95.5, -29}}, textString = "%name", fontSize = 12)}),
    Diagram(coordinateSystem(grid = {1, 1})));
end ElectricalVehiclePool;
